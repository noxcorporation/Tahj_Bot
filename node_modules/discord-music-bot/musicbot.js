	...
	if(message.channel.type === "dm" && message.author.id !== bot.user.id) { //Message received by DM
		...
		if(message.isMentioned(bot.user)) {
			...
			if(message_text[0] == '!') { //Command issued
				handle_command(message, "search never gonna hit those notes");
...
<<<<<<< HEAD
				...
				for (i = 0; i < exprLenght; i++){
					if (expr[i].test(message_text)){
							handle_command(message, "search initial d deja vu");
					}
=======
			...
			var exprLength = expr.length;
			for (i = 0; i < exprLenght; i++){
				if (message_text.match(expr[i])){
						search_video(message, "initial d deja vu");
>>>>>>> parent of 21ac894... still attempt #2 with more git voodoo
				}
			}
		}
	}
...
function play_next_song() {
	...
	var audio_stream = ytdl("https://www.youtube.com/watch?v=" + video_id, var options = {highWaterMark: 32768});
