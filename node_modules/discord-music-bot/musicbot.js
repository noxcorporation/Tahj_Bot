...
var secret_search = "initial d deja vu"; //TODO Read from file/DB
var expr = [/takumi/, /drift/, /eurobeat/]; //regexp TODO Read from file/DB
...
var commands = [
  ...
  {
    command: "version",
		description: "Return the bot version",
		parameters: [],
		execute: function (message, params) {
			
			message.reply(bot.user.username, " version : ", version");
			
			.catch((err) => {
				message.reply('Error: Unable to read version');
				console.log('Error on version command:', err); 
      });
		}
  },
  ...
  {
    command: "secret",
		description: "Set something for secret feature",
		parameters: ["Secret"],
		execute: function (message, params) {
			...
			parameters.shift();
			secret_search = parameters.join(" "); //TODO write to secret_search file
			
			message.reply('Secret parameter set.');
			
			.catch((err) => {
				message.reply('Error: Unable to read secret');
				console.log('Error on secret command:', err); 
      });
		}
  },
  ...
  {
    command: "keywords",
		description: "Add keywords for secret feature",
		parameters: ["add or remove, keyword, no parameter for listing keywords"],
		execute: function (message, params) {
			...
			if (action == "add"){
				var i = expr.lenght;
				expr[i] = "/" + keyword + "/";
				message.reply(keyword, ' added to the list of keywords.');
				//TODO write keywords file
			}
			else if (action == "remove"){
				expr.forEach(function(item, index){
					if(item.test(keyword)){
						expr.splice(index-1,index);
						message.reply(keyword, ' removed from the list of keywords.');
						//TODO write keywords file
						break;
					}
				}
				...
				message.reply(expr.join('\\n'));
			}
			
			.catch((err) => {
				message.reply('Error: Unable to read keywords');
				console.log('Error on keywords command:', err); 
      });
		}
  }
...
bot.on("message", message => {
	if(message.channel.type === "dm" && message.author.id !== bot.user.id) { //Message received by DM
		...
		if(message.isMentioned(bot.user)) {
			...
			} else if (message.author.id !== bot.user.id) {eurobeat(message);}
		}
	} else if (message.channel.type === "text" && message.author.id !== bot.user.id) {eurobeat(message);}
});
...
function play_next_song() {
	...
	var options = {highWaterMark: 32768, retries: 15, filter: "audioonly"};
	var audio_stream = ytdl("https://www.youtube.com/watch?v=" + video_id, options);
...
function search_video(message, query, mute = false) {
	request("https://www.googleapis.com/youtube/v3/search?part=id&type=video&q=" + encodeURIComponent(query) + "&key=" + yt_api_key, (error, response, body) => {
		...
		if("error" in json) {
			...
			add_to_queue(json.items[0].id.videoId, message, mute);
		}
	})
}
...
function eurobeat(message) {
	var message_text = message.content;
	var exprLength = expr.length;
	for (i = 0; i < exprLength; i++){
		if (expr[i].test(message_text)){
			search_video(message, secret_search, true);
			break;
		}
	}
}
...
exports.run = function(server_name, text_channel_name, voice_channel_name, aliases_path, token) {
	...
	bot.login(token);
	text_channel.send('Ready to process commands!');
}
